...
def correct_artefacts(wsh):
    ...
    for rem in to_remove:
        rem_im = wsh==rem #cells to be removed are marked with 1
        rem_cont = dilation(rem_im) & ~rem_im #dilated image sets neighboring pixels to 1
        #rem_cont conatains the dilated regions (ie non cell region)
...
def cell_merge(wsh, pred):
    ...
    objs = np.zeros((wsh.max()+1,wshshape[0],wshshape[1]), dtype=bool) #mask for each cell
    ...
    for obj1 in range(wsh.max()):
        ...
        for obj2 in range(obj1+1,wsh.max()):
            ...
            if (do_box_overlap(obj_coords[obj1,:], obj_coords[obj2,:])
                ...
                if len(border_pred) < 40:

                    #test
                    #print("------cell overlap detection voided-----")
                    #print("Border Length: " + str(len(border_pred)))
                    #print("\t obj1: " + str(obj1))
                    #print("\t obj2: " + str(obj2))
                    continue
                ...
                # merge cells
                if top_border_height / top_border_area >= .90:

                    #test
                    #print("-------Cell Overlap Detected: Above Threshold")
                    #print("\t obj1: " + str(obj1))
                    #print("\t obj2: " + str(obj2))
                ...
                #else:
                    #test
                    #print("-----Overlap Detected Error: Border Pixels Blow Threshold------")
                    #print("\t Ratio: " + str(top_border_height / top_border_area))
                    #print("\t obj1: " + str(obj1))
                    #print("\t obj2: " + str(obj2))
